...
class Kohana_Database_PDO_SQLite_Alter {
	...
	private function handle_add_primary_key($queries)
	{
		...
		$query_obj = DB::query(Database::SELECT, "SELECT sql FROM sqlite_master WHERE tbl_name='$tbl_name'")
			->as_object()
			->execute();
	...
	private function handle_drop_primary_key($queries)
	{
		$tokenized_query = $queries;
		$temp_table = 'temp_' . $tokenized_query['table_name'];
		$query_obj = DB::query(Database::SELECT, "SELECT sql FROM sqlite_master WHERE tbl_name='{$tokenized_query['table_name']}'")
			->as_object()
			->execute();
	...
	private function handle_change_command($queries)
	{
		...
		$col_obj = DB::query(Database::SELECT, "SHOW COLUMNS FROM {$tokenized_query['table_name']}")
			->as_object()
			->execute();
