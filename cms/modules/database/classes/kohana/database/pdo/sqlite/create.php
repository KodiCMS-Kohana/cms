...
class Kohana_Database_PDO_SQLite_Create {
	...
	public function rewrite_query($query)
	{
		...
		{
			// we manipulate CREATE INDEX query in PDO class
		...
		{
			// if use foreign key constraint, trigger will be needed.
	...
	private function rewrite_comments()
	{
		...
		$this->_query = preg_replace("/#/", "--", $this->_query);
		$this->_query = preg_replace("/COMMENT '(.*?)'(,)*/", "$2 -- $1", $this->_query);
	}
	...
	private function _rewrite_unique_key($matches)
	{
		...
		$results = DB::query(Database::SELECT, "SELECT name FROM sqlite_master WHERE type='index'")
			->as_object()
			->execute();
	...
	{
		$this->_query = preg_replace("/FULLTEXT\\sKEY\\s/", "INDEX", $this->_query); // TODO fulltext index
