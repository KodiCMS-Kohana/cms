 ...
 */
class Model_Search_Stemmer_Russian extends Model_Search_Stemmer_Decorator {
	...
	function rv($word)
	{
		...
		for ($i = 0; $i < strlen($word); $i+=self::CHAR_LENGTH)
		{
			if ($flag == 1)
			{
				$rv .= substr($word, $i, self::CHAR_LENGTH);
			}
			else
			{
	...
	function step2($word)
	{
		return substr($word, -self::CHAR_LENGTH, self::CHAR_LENGTH) == 'и' 
			? substr($word, 0, strlen($word) - self::CHAR_LENGTH) 
			: $word;
	}
