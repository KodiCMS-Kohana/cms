<div class="form-group">
	<label class="control-label col-md-3" for="allow_html"><?php echo __('Allow HTML tags'); ?></label>
	<div class="col-md-9">
		<?php echo Form::checkbox('allow_html', 1, $field->allow_html == 1, array('id' => 'allow_html' )); ?>
	</div>
</div>
<div class="form-group">
	<label class="control-label col-md-3" for="text_filter_html"><?php echo __('Filter HTML tags'); ?></label>
	<div class="col-md-9">
		<?php echo Form::checkbox('filter_html', 1, $field->filter_html == 1, array('id' => 'text_filter_html' )); ?>
	</div>
</div>

<div class="form-group" id="text_allowed_tags_container">
	<label class="control-label col-md-3" for="text_allowed_tags"><?php echo __('Allowed tags'); ?></label>
	<div class="col-md-9">
		<?php echo Form::textarea('allowed_tags', $field->allowed_tags, array('id' => 'text_allowed_tags', 'rows' => 2 )); ?>
	</div>
</div>

<hr />

<div class="form-group">
	<label class="control-label col-md-3" for="rows"><?php echo __('Rows'); ?></label>
	<div class="col-md-9">
		<?php echo Form::input( 'rows', $field->rows, array(
			'class' => 'input-mini', 'id' => 'rows'
		) ); ?>
	</div>
</div>

<script type="text/javascript">
	$(function() {
		$(document).on('change', '#text_filter_html', function() {
			if($(this).is(':checked')) {
				$('#text_allowed_tags_container').show();
			} else {
				$('#text_allowed_tags_container').hide();
			}
		}).change();
	})
</script>