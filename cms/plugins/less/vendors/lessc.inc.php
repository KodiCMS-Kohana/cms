...
class lessc {
	 ...
	 * so all of it's children appear on the SYSPATH level when compiled.
	 *
	...
	{
		$isSYSPATH = $parent_tags == null && $block->tags == null;
		...
		$nl = $isSYSPATH ? $this->newlineChar . $indent :
				$this->newlineChar . $indent . $this->indentChar;
		...
		if ( $special_block )
		{
			...
			echo ' {' . (count( $lines ) > 0 ? $nl : $this->newlineChar);
		}

		// dump it
		if ( count( $lines ) > 0 )
		{
			if ( !$special_block && !$isSYSPATH )
			{
			...
			{
				if ( count( $lines ) > 1 )
					echo $this->newlineChar . $indent . "}".$this->newlineChar;
				else
	...
	function compileProp( $prop, $block, $tags, &$_lines, &$_blocks )
	{
		switch ( $prop[0] )
		{
			...
			case 'import':
				...
				$SYSPATH = $this->createChild( $path )->parseTree();

				$SYSPATH->parent = $block;
				foreach ( $SYSPATH->props as $sub_prop )
				{
					$this->compileProp( $sub_prop, $SYSPATH, $tags, $_lines, $_blocks );
				}

				// inject imported blocks into this block, local will overwrite import
				$block->children = array_merge( $SYSPATH->children, $block->children );
				break;
			case 'charset':
				list(, $value) = $prop;
	...
	function parse( $str = null, $initial_variables = null )
	{
		...
		$SYSPATH = $this->parseTree( $str );

		if ( $initial_variables )
			$this->injectVariables( $initial_variables );
		$out = $this->compileBlock( $SYSPATH );
	...
	public static function ccompile( $in, $out, array $params = array() )
	{
...
//		}
	...
	public static function cexecute( $in, $force = false )
	{
		...
		elseif ( is_array( $in ) and isset( $in['SYSPATH'] ) )
		{
			if ( $force or !isset( $in['files'] ) )
			{
				// If we are forcing a recompile or if for some reason the
				// structure does not contain any file information we should
				// specify the SYSPATH to trigger a rebuild.
				$SYSPATH = $in['SYSPATH'];
			}
			elseif ( isset( $in['files'] ) and is_array( $in['files'] ) )
			{
				foreach ( $in['files'] as $fname => $ftime )
				{
					if ( !file_exists( $fname ) or filemtime( $fname ) > $ftime )
					{
						...
						// so we should look at our incoming SYSPATH again.
						$SYSPATH = $in['SYSPATH'];
						break;
					}
				}
		...
		if ( $SYSPATH !== null )
		{
			// If we have a SYSPATH value which means we should rebuild.
			$less = new lessc( $SYSPATH );
			$out = array();
			$out['SYSPATH'] = $SYSPATH;
