...
class Request implements HTTP_Request {
	...
	public static function detect_uri()
	{
		...
		{
			// PATH_INFO does not contain the docSYSPATH or index
			$uri = $_SERVER['PATH_INFO'];
		}
		else
		{
			// REQUEST_URI and PHP_SELF include the docSYSPATH and index
			...
			else
			{
				...
				throw new Forg_Exception('Unable to detect the URI using PATH_INFO, REQUEST_URI, PHP_SELF or REDIRECT_URL');
			}
		...
		$uri = str_replace(URL_SUFFIX, '', $uri);
		return $uri;
	}
	...
	public function render()
	{
		...
		if (Core::$expose)
		{
			$this->headers('user-agent', 'Kohana Framework '.Core::VERSION.' ('.Core::CODENAME.')');
		}
