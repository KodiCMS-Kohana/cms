...
class Core_Global_Exception extends Exception {
	...
	{
		require_once APPPATH.'classes'.DIRECTORY_SEPARATOR.'i18n'.EXT;

		if (defined('E_DEPRECATED'))
		{
			// E_DEPRECATED only exists in PHP >= 5.3.0
			Core_Exception::$php_errors[E_DEPRECATED] = 'Deprecated';
		}
	...
	public static function handler(Exception $e)
	{
		try
		{
			...
			if ( ! headers_sent())
			{
				...
				header('Content-Type: '.Core_Exception::$error_view_content_type.'; charset='.Core::$charset, TRUE, $http_header_status);
			}

			// Start an output buffer
			ob_start();

			$view_file = APPPATH.'views'.DIRECTORY_SEPARATOR.'layouts'.DIRECTORY_SEPARATOR.Core_Exception::$error_view.EXT;
			...
			{
				throw new Core_Exception('Error view file does not exist: layouts/:file', array(
					':file' => Core_Exception::$error_view,
				));
			}
