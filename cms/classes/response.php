 ...
 * @category   Base
 * @author     Core Team
 * @copyright  (c) 2008-2011 Core Team
 * @license    http://Corephp.com/license
 * @since      3.1.0
 */
class Response implements HTTP_Response {
	...
	public function status($status = NULL)
	{
		...
		{
			throw new Forg_Exception(__METHOD__.' unknown status value : :value', array(':value' => $status));
		}
	}
	...
	public function send_file($filename, $download = NULL, array $options = NULL)
	{
		...
		if ($filename === TRUE)
		{
			if (empty($download))
			{
				throw new Forg_Exception('Download name must be provided for streaming files');
			}
		...
		if ( ! is_resource($file))
		{
			throw new Forg_Exception('Could not read file to send: :file', array(
				':file' => $download,
			));
		}
	...
	public function render()
	{
		...
		// If Core expose, set the user-agent
		if (Core::$expose)
		{
